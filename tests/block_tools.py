...
from hydrangea.daemon.keychain_proxy import connect_to_keychain_and_validate, wrap_local_keychain
...
from hydrangea.plotting.create_plots import create_plots, PlotKeys
...
from hydrangea.consensus.coinbase import create_puzzlehash_for_pk, create_farmer_coin, create_pool_coin
...
from hydrangea.plotting.util import PlotsRefreshParameter, PlotRefreshResult, PlotRefreshEvents, parse_plot_info
from hydrangea.plotting.manager import PlotManager
from hydrangea.server.server import ssl_context_for_server
from hydrangea.types.blockchain_format.classgroup import ClassgroupElement
from hydrangea.types.blockchain_format.coin import Coin, hash_coin_list
    ...
    async def setup_keys(self):
        if self.local_keychain:
            ...
            self.keychain_proxy = await connect_to_keychain_and_validate(
                self.root_path, log, user="testing-1.8.0", service="hydrangea-testing-1.8.0"
            )
        ...
        if len(self.pool_pubkeys) == 0 or len(self.farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `hydrangea generate keys`")
...
def get_plot_dir() -> Path:
    cache_path = Path(os.path.expanduser(os.getenv("HYDRANGEA_ROOT", "~/.hydrangea/"))) / "test-plots"
    mkdir(cache_path)
    return cache_path
