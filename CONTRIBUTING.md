...
In the event an emergency fix is required for the release version of Hydrangea, members of the Hydrangea organization will create a feature branch from the current release branch `1.0.0`.
...
. ./activate
pip install ".[dev]"
black hydrangea tests && mypy hydrangea tests && flake8 hydrangea tests
...
We provide a [pre-commit configuration](https://github.com/HydrangeaBlockchain/hydrangea-blockchain/blob/main/.pre-commit-config.yaml) which triggers several useful
...
6. Install these linters https://github.com/HydrangeaBlockchain/hydrangea-blockchain/tree/main/.github/linters

## Testnets and review environments

With the launch of `1.0.0` we will begin running an official `testnet`.
Prior to the release of `1.1.0` there will be two running test nets. `testnet` and `transaction-beta-testnet`. The `transaction-beta-testnet` testnet will be a beta of the pending 1.1 release, which will enable transactions on the hydrangea blockchain.
